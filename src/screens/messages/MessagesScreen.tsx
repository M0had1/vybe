...
export default function MessagesScreen({ navigation }: any) {
  ...
  const convList = friends.map((f: any) => {
    ...
    return { friend: f, lastMessage: last, unread: unreadCount(f.id) };
  }).sort((a: any, b: any) => {
  ...
  return (
    <View style={[s.container, { paddingTop: insets.top }]}>
      <View style={s.header}><Text style={s.headerTitle}>Messages</Text><TouchableOpacity style={s.newBtn}><Text style={s.newIcon}>✏️</Text></TouchableOpacity></View>
      <FlatList data={convList} keyExtractor={(item: any) => item.friend.id}
        renderItem={({ item }: any) => (
          <TouchableOpacity onPress={() => navigation.navigate('Chat', { friend: item.friend })} style={s.convItem}>
            ...
            <View style={s.convInfo}>
              <View style={s.convHeader}><Text style={s.convName}>{item.friend.name}</Text>{item.lastMessage && <Text style={s.convTime}>{getTimeAgo(item.lastMessage.time)}</Text>}</View>
              <Text style={[s.convPreview, item.unread > 0 && s.convPreviewUnread]} numberOfLines={1}>{item.lastMessage ? (item.lastMessage.from === user.id ? 'You: ' : '') + item.lastMessage.text : 'Start a conversation'}</Text>
            </View>
            {item.unread > 0 && <View style={s.unreadBadge}><Text style={s.unreadText}>{item.unread}</Text></View>}
          </TouchableOpacity>
        )}
        ItemSeparatorComponent={() => <View style={s.separator} />}
        ListEmptyComponent={<View style={s.emptyState}><Text style={s.emptyEmoji}>💬</Text><Text style={s.emptyTitle}>No messages yet</Text></View>} />
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  convTime: { color: colors.textTertiary, fontSize: 11 },
