    ...
    <View style={[s.container, { paddingTop: insets.top }]}>
      ...
      <View style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.headerStatus}>{friend.mood?.emoji} {friend.mood?.text}</Text>
        </View>
        <TouchableOpacity style={s.moreBtn}><Text style={s.moreIcon}>⋯</Text></TouchableOpacity>
      </View>

      {/* Screenshot notice */}
      <View style={s.screenshotNotice}>
        <Text style={s.screenshotText}>🔒 Screenshot detection active</Text>
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={[s.inputBar, { paddingBottom: insets.bottom + 8 }]}>
          ...
          <TouchableOpacity onPress={handleSend} style={s.sendBtn}>
            <Text style={s.sendIcon}>{text.trim() ? '➤' : '🎤'}</Text>
          </TouchableOpacity>
        </View>
...
const s = StyleSheet.create({
  ...
  headerName: { color: colors.text, fontSize: font.md, fontWeight: '700' },
  headerStatus: { color: colors.textSecondary, fontSize: font.xs },
  ...
  screenshotText: { color: colors.textTertiary, fontSize: font.xs },
  bubbleRow: { marginBottom: 8, flexDirection: 'row' },
  bubbleRowMe: { justifyContent: 'flex-end' },
  bubble: { maxWidth: '75%', paddingHorizontal: 14, paddingVertical: 10, borderRadius: radius.xl },
  bubbleMe: { backgroundColor: colors.primary, borderBottomRightRadius: 4 },
  bubbleThem: { backgroundColor: colors.surface, borderBottomLeftRadius: 4 },
  bubbleText: { color: colors.text, fontSize: font.md, lineHeight: 20 },
